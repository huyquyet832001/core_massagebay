...
<div class="row margin0">
	...
	<div class="col-md-10 col-xs-12">
		...
		<input value="<?php echo htmlspecialchars($valueDb); ?>" name="<?php echo $field['name'] ?>" type="hidden"/>
		<input type="text" class="search<?php echo $field['name'] ?>" style="max-width: 500px" placeholder="<?php echo alang('TYPING_SEARCH') ?>">
		<ul class="listitem multiselect padding0 scrollbar listitem<?php echo $field['name'] ?>">
			<?php 
				...
				if($source==="static"){
					...
					$valueput = $valueDb;
					printRecursiveMultiSelect2(0,$arr,$valueput);
				}
			 ?>
		</ul>
		<script type="text/javascript">
			function changeInfo(){
				var inputCheck = $('.listitem<?php echo $field["name"] ?> li input:checked');
				var inputText = $('.listitem<?php echo $field["name"] ?> li input:checked').next().next();
				var str = '{';
				for (var i = 0; i < inputCheck.length; i++) {
					var itemId = inputCheck[i];
					var itemLink = inputText[i];
					str += '"'+$(itemId).val()+'": ';
					str += '"'+$(itemLink).val()+'"';
					if(i<inputCheck.length-1){
						str+=", ";
					}
				};
				str += '}';
			...
			$(function() {
				$('body').on('click', '.listitem<?php echo $field["name"] ?> li input[type="checkbox"]', function(event) {
					changeInfo();
				});
				$('body').on('input', '.listitem<?php echo $field["name"] ?> li input[type="text"]', function(event) {
					changeInfo();
				});
