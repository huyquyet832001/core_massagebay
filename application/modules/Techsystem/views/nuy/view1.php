              ...
              <?php if($table['help']==1){ ?>
                 <td align="center">
                  ...
                  <?php if($table['insert']==1) { ?>
                     <td align="center">
                      <a title="<?php echo alang("INSERT") ?>" class="toolbar btn btn-info" href="<?php echo base_url().'Techsystem/'.'insert/'.$table['name']?>?return=<?php echo base64_encode(current_full_url()) ?>"><i class="icon-plus"></i>&nbsp;
                          <?php echo alang("INSERT") ?></a>
                      </td>
                      <?php } ?>
                      <?php if($table['delete']==1) { ?>
                         <td align="center">
                          <a onclick="deleteAll();return false;"  href="javascript:deleteAll();" title="<?php echo alang("DELETE") ?>" class="deleteall toolbar btn btn-info"><i class="icon-trash"></i>&nbsp;
               ...
               <td align="center">
                ...
                <?php if($table['insert']==1){ ?>
                   <td align="center">
                    ...
                    <?php if($table['delete']==1){ ?>
                       <td align="center">
                        <a onclick="deleteAll();return false;"  href="javascript:deleteAll();"  title="<?php echo alang("DELETE") ?>" class="deleteall toolbar btn btn-info" href=""><i class="icon-trash"></i>&nbsp;
                            <?php echo alang("DELETE") ?></a>
                        </td>
                        <?php } ?>
                    </tr>
...
<div class="controlsearch row margin0">
   ...
   ?>
   <button type="submit" name="submit" value="<?php echo alang("FILTER") ?>" class="btn"><i class="icon-filter"></i> <?php echo alang("FILTER") ?></button>
   <button type="reset" name="submit" onclick="window.location.href='Techsystem/view/<?php echo $table['name'] ?>'" class="btn"><i class="icon-refresh"></i><?php echo alang("REFRESH") ?></button>
   <button type="button" name="submit" data-toggle="modal" data-target="#advanceSearch" class="btn"><i class="icon-search"></i><?php echo alang("ADVANCE_SEARCH") ?></button>
</div> 
</form>
</div>
</div>
        ...
        <thead class="cf">
          <tr>
            ...
            <th align="left" scope="col">
                <input type="checkbox" name="cball" dt-delete="" class="cball" onclick="">
            </th>
            <?php 
            foreach ($titles as $key => $item) {
...
<tbody>
    ...
    $i =0; foreach ($lstData as $key => $item) {
      ...
      <tr> 
          <td><?php echo $i; ?></td>   
          <td><input type="checkbox" class="cbone" dt-value="<?php echo $item['id'] ?>" name="cb<?php echo $item['id'] ?>" onclick=""></td>
          <?php 
          foreach ($titles as $val) {
           ...
           ?>
            ...
            <?php if(isset($item) && isset($item['slug'])){ ?>
                ...
                <?php }if($table['insert']){ ?>
                    <a href="Techsystem/copy/<?php echo $table['name'] ?>/<?php echo $item['id'] ?>?return=<?php echo base64_encode(current_full_url()) ?>&test=1" class="edit fnc" ><i class="icon-copy"></i></a>
                    <?php } ?>
                    <?php if($table['edit']){ ?>
                        <a href="Techsystem/edit/<?php echo $table['name'] ?>/<?php echo $item['id'] ?>?return=<?php echo base64_encode(current_full_url()) ?>" class="edit fnc" ><i class="icon-edit"></i></a>
                        <?php } ?>
                        <?php if($table['delete']){ ?>
